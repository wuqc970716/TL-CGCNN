...
#parser.add_argument('--h-fea-len', default=116, type=int, metavar='N',
#                    help='number of hidden features after pooling')
parser.add_argument('--fc-fea-len', default=68, type=int, metavar='N',
                    help='number of hidden features in the added full-connect layers')
...
if args.task == 'regression':
    ...
    best_mae_error = 0.
if args.n_h<2:
    print("n_h should be at least 2")
    raise SystemExit
...
def main():
    ...
    if args.n_h>2:
        i=1
        while i <= args.n_h-2:
            model.fcs[i] = nn.Linear(in_features=args.fc_fea_len, out_features=args.fc_fea_len, bias=True)
            i+=1
    
    model.fc_out = nn.Linear(in_features=args.fc_fea_len, out_features=1, bias=True)
...
def save_model(model, is_best, filename='checkpoint.pth'):
